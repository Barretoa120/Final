...
public final class MyMap<K,V> implements Map<K,V> {
  ...
  private class Node<Character,V>{
    ...
    HashMap<K,Node<Character,V>> nexts=new HashMap<>();
    ...
    */
    ...
    //to returns next
    public Node<Character,V> getNext(K nextkey){ //change K to character?
...
//be able to insert word take out word view all words in tree
  ...
  public Collection<V> values() {
    ...
    public V put(final K key, final V value) { //put is the add function!! //returns old value after insertion v value
      Node<Character,V> n = new Node<Character,V>();
      n.setVal(value);
    ...
    public V get(final Object key) { //how to track index with multiple nodes
      /*
      Node temp = null;
      temp = head;
